...
class RetriveInfo : public Communicate_ {
    ...
    char* get_info(const char* table) {
        ...
        //    return NULL;
        //}
        char *info_container = recv_json(ssl);
    ...
    char* list_tasks(const char *id) {
        ...
        char *info_json = cJSON_Print(info); // Smaller payload
        if (!info_json) {
            cJSON_Delete(info);
            //free(info_container);
            return NULL;
