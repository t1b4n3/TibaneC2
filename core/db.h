...
char *get_task(int task_id) {
    char query[1024];
    snprintf(query, sizeof(query), "SELECT command where task_id = %d", task_id);
    if (mysql_query(con, query)) {
        fprintf(stderr, "%s\n", mysql_error(con));
        mysql_close(con);
    }
...
int check_tasks_queue(char *agent_id) {
    char query[1024];
    snprintf(query, sizeof(query), "SELECT task_id, status WHERE agent_id = '%s'", agent_id);
    if (mysql_query(con, query)) {
        fprintf(stderr, "%s\n", mysql_error(con));
        mysql_close(con);
        return -1;
...
int AgentsTable(struct db_agents args) {
    char query[1024];
